...
public class HotelEditForm extends FormLayout {
    ...
    private void preapareFied () {
        ...
        binder.forField(paymentMethod).withValidator(paymentValidator()).bind(Hotel::getPaymentMethod, Hotel::setPaymentMethod);
    ...
    private Validator<? super PaymentMethod> paymentValidator () {
        return (value, context) -> {
            if (value.getCard() < 0 || value.getCard() > 100) return ValidationResult.error("Percent must be between 0 and 100");
            return ValidationResult.ok();
        };
    }
